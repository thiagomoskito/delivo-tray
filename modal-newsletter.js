  ...
  if (!localStorage.getItem('newsletter_checkout_seen')) {
    ...
    modal.innerHTML = '<div style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.8);z-index:999999;display:flex;align-items:center;justify-content:center;">' +
        '<div style="background:#fff;padding:30px;width:90%;max-width:400px;border-radius:10px;position:relative;text-align:center;font-family:sans-serif;">' +
          '<span id="close-modal-news" style="position:absolute;right:15px;top:10px;cursor:pointer;font-size:25px;font-weight:bold;">&times;</span>' +
          '<h2 style="margin-top:10px;">Ganhe 10% de desconto</h2>' +
          '<p>Assine nossa newsletter antes de finalizar!</p>' +
          '<input type="email" id="modal-email" placeholder="Seu e-mail" style="width:100%;padding:12px;margin:15px 0;border:1px solid #ccc;border-radius:4px;box-sizing:border-box;">' +
    ...
    document.getElementById('btn-save-news').onclick = function() {
      ...
      if(email && email.indexOf('@') > -1) {
        ...
        alert('Obrigado! Use o cupom: BEMVINDO10');
        modal.remove();
      }
    };
  }
